...
board.addEventListener("click", eventHandler);
...
function eventHandler(e) {
  if (e.target.classList.contains("cell")) {
    ...
    e.target.innerText = move === "player1" ? "X" : "O";
    move = move === "player1" ? "player2" : "player1";
    if (checkWin()) {
      board.removeEventListener("click", eventHandler);
    }
  }
}
...
function checkWin() {
  ...
  for (const line of possibleEnds) {
    if (line.every((cell) => cell.innerHTML === "X")) {
      board.classList.add("win");
      return true;
    }
    if (line.every((cell) => cell.innerHTML === "O")) {
      board.classList.add("lose");
      return true;
    }
  }
  // const win = possibleEnds[1].every((cell) => cell.innerHTML === "X");

  // const win = possibleEnds.forEach((line) =>
  //   line.every((cell) => cell.innerText === "X")
  // );
  // console.log(win);
}
